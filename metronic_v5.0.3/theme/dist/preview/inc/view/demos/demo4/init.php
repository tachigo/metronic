...
class Demo4Init {
	...
	private static function initLayout() {
		...
		Page::addAttr('body', 'style', 'background-image: url(' . Page::getMediaImgPath() . 'bg/bg-1.jpg' . ')');	
	}
	...
	private static function initHeader() {		
		// header fixed mode
		if (Page::getOption('layout', 'header/self/fixed/desktop') == true && Page::getOption('layout', 'content/fixed/desktop') == false) {
			Page::addClass('body', 'm-header--fixed');
		} else {
			Page::addClass('body', 'm-header--static');
		}

		if (Page::getOption('layout', 'header/self/fixed/mobile') == true && Page::getOption('layout', 'content/fixed/mobile') == false) {
	...
	private static function initContent() {
		if ( Page::getOption('layout', 'content/fixed/desktop') ) {
			Page::addClass('body', 'm-body--fixed');
		}

		if ( Page::getOption('layout', 'content/fixed/mobile') ) {
			Page::addClass('body', 'm-body--mobile-fixed');
		}			
	}
